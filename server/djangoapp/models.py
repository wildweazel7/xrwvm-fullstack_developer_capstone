...
class CarModel(models.Model):
    ...
    color = models.CharField(max_length=255, choices=mColorChoice, help_text="Offered colors of the model")
    doors = models.CharField(max_length=255, choices=mDoorChoice, help_text="Number of doors possible", default="Five")
    extras = models.CharField(max_length=255, choices=mExtraChoice, help_text="List of extras",blank=True, default='NONE')
    price = models.DecimalField(max_digits=10, decimal_places=2, help_text="Price in USD",blank=True, default=10000)
    dealer_id = models.IntegerField(blank=True,null=True,default=1)

    def __str__(self):
        # Returns the name of the car make when it's printed
        return f"{self.car_make.name} {self.name} - {self.extras} - {self.color} - {self.doors} - {self.price}"
